...
describe('/api/topics/:topic', () => {
  describe('DELETE', () => {
    it('responds 204 and deletes the specified topic', () => {
      return request.delete('/api/topics/mitch').expect(204);
    });
    it('responds 404: Item not found when given a valid but non-existent article_id', () => {
      return request
        .delete('/api/topics/hello')
        .expect(404)
        .then(({ body: { msg } }) => {
  ...
  describe('INVALID METHODS', () => {
    it('responds 405: Method not allowed for any unexpected method', () => {
      const invalidMethods = ['patch', 'put', 'get', 'post'];
      const promises = invalidMethods.map(method => {
        return request[method]('/api/topics/topic')
          .expect(405)
          .then(({ body: { msg } }) => {
            expect(msg).to.equal('Method not allowed!');
          });
